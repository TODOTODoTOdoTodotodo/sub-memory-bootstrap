...
def build_agents_block() -> str:
    return (
        ...
        "- Use `store_memory` only when the user explicitly asks to persist "
        "something immediately, or when the current runtime is not already storing "
        "the final exchange automatically.\n"
        "- Use `reinforce_memory` only when recalled memory materially influenced "
        "the final answer or code change.\n"
