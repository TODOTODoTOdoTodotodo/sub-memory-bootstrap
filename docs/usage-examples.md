...
이 문서는 GitHub 저장소 `sub-memory-bootstrap`을 clone한 뒤 실제로 사용하는 예시를 모아 둔 문서입니다. 예시는 로컬 에이전트 실행, CLI 연동, 그리고 Codex skill 사용 예시까지 포함합니다.
...
```text
Use sub-memory-bootstrap to set up this cloned repository locally, verify the install, and show me the exact Codex/Gemini/Claude MCP snippets for this machine.
```

```text
sub-memory-bootstrap을 사용해서 clone한 이 저장소를 로컬에 설치하고, 설치 검증까지 한 뒤, 현재 머신 기준 Codex/Gemini/Claude MCP 설정 스니펫을 보여줘.
```

### 예시 B: 재검증

```text
Use sub-memory-bootstrap to validate the existing installation and tell me whether the local stdio MCP server is ready.
```

```text
sub-memory-bootstrap으로 기존 설치 상태를 다시 점검하고, 로컬 stdio MCP 서버를 바로 쓸 수 있는지 알려줘.
```
...
다른 사람과 공유할 때는 로컬 절대경로를 전달하지 말고, 아래처럼 저장소 URL과 기준 커밋을 전달하는 것이 안전합니다.
...
팀 문서에는 보통 아래 항목만 적으면 충분합니다.

1. GitHub 저장소 URL
2. 기준 commit 또는 release tag
3. 권장 Python 버전
4. bootstrap 실행 명령
5. 테스트 확인 명령
