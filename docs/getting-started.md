...
- `.codex/config.toml`은 전역 파일이 아니라 이 저장소 전용 project-local 설정입니다.
- 따라서 `sub-memory-bootstrap`을 실행한 저장소마다 각자의 `.codex/config.toml`이 생성됩니다.
- 새 Codex 세션은 현재 저장소 루트에서 시작해야 이 project-local 설정을 읽습니다.
...
이 bootstrap 흐름은 저장소 안에 project-local 설정을 만들기 때문에, 보통 아래 항목은 Git에서 제외하는 편이 안전합니다.
...
최소 기준으로는 `.codex/` 전체를 ignore 하면 됩니다. 현재 bootstrap이 생성하는 project-local Codex 파일은 `.codex/config.toml`입니다.

### 0-6. AGENTS.md에 자동으로 추가되는 내용

bootstrap이 실행되면 `AGENTS.md`에 managed block이 들어갑니다. 핵심 규칙은 아래와 같습니다.

- `get_memory_status`
  - prior memory가 중요할 수 있는 구현, 디버깅, 리뷰, 계획 작업 전 확인
...
- compact
  - 멀티턴이 길어지면 raw transcript 대신 짧은 working summary와 recall 조합으로 이어감
- 세션 안내
  - `sub_memory` tools가 안 보이면 `.codex/config.toml`이 project-local에 있다는 점과 새 Codex 세션 시작 필요성을 설명

이 block은 `<!-- BEGIN SUB-MEMORY MCP RULES -->` 와 `<!-- END SUB-MEMORY MCP RULES -->` 사이에 관리됩니다.
